...
public class SlimeSpawnState : EnemyState
{
    public SlimeSpawnState(Enemy enemyContext) : base(enemyContext) { }

    protected override void OnEnter()
    {
        //Debug.Log("Enter Spawn");
        _enemyContext.StateMachine.SwitchState(_enemyContext.StateCache.RequestState(EnemyStateCache.EnemyStates.SlimeIdle));
    }
}

public class SlimeIdleState : EnemyState
{
    private readonly SlimeData _configFile;
    ...
    private void CheckPlayerDistance()
    {
        ...
        {
            _enemyContext.StateMachine.SwitchState(_enemyContext.StateCache.RequestState(EnemyStateCache.EnemyStates.SlimeChase));
            return;
        }
    }

    public SlimeIdleState(SlimeData configFile, Enemy enemyContext) : base(enemyContext)
    {
    ...
    {
        CheckDeath(_enemyContext.StateCache.RequestState(EnemyStateCache.EnemyStates.SlimeDeath));
    }
    protected override void OnEnter()
    {
...
public class SlimeChaseState : EnemyState
{
    private void CheckNavMeshDistance()
    {
        ...
        if (agent.remainingDistance <= agent.stoppingDistance)
        {
            if (!agent.hasPath || agent.velocity.sqrMagnitude == 0f)
                _enemyContext.StateMachine.SwitchState(_enemyContext.StateCache.RequestState(EnemyStateCache.EnemyStates.SlimeCharge));
        }
    }
    ...
    public SlimeChaseState(Enemy enemyContext) : base(enemyContext) { }
    protected override void OnEnable()
    {
        TickManager.Instance.GetTimer(0.2f).Tick += Tick;
    }
...
public class SlimeChargeState : EnemyState
{
    ...
    private IEnumerator ChargeAttackCoroutine()
    {
        ...
        if (TempoConductor.Instance.IsOnBeat()) 
            _enemyContext.StateMachine.SwitchState(_enemyContext.StateCache.RequestState(EnemyStateCache.EnemyStates.SlimeAttack));
        else 
    ...
    public SlimeChargeState(SlimeData configFile,Enemy enemyContext) : base(enemyContext) 
    {
        _configFile = configFile;
    }
...
public class SlimeAttackState : EnemyState
{
    ...
    public SlimeAttackState(SlimeData configFile, Enemy enemyContext) : base(enemyContext) 
    {
        _configFile = configFile;
    }
...
public class SlimeStaggerState : EnemyState
{
    ...
    public SlimeStaggerState(SlimeData configFile, Enemy enemyContext) : base(enemyContext)
    {
        _configFile = configFile;
    }
...
{
    public SlimeDeathState(Enemy enemyContext) : base(enemyContext) { }
